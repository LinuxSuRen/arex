...
const AppPaneLayout: FC<AppPaneLayoutProps> = ({
  ...
  const [panePrimarySize, setPanePrimarySize] = useState(26);
  const [paneSecondarySize, setPaneSecondarySize] = useState(74);
  ...
  function populatePaneEvent() {
    ...
    if (verticalPaneData) {
      ...
      const p =
        calculatePercentage(mainPane?.size || 0, sidebarPane?.size || 0) * 100;
  ...
  return (
    <div css={css``}>
      ...
      >
        <Allotment.Pane preferredSize={`${panePrimarySize}%`}>
          {primary}
        </Allotment.Pane>
        <Allotment.Pane preferredSize={`${paneSecondarySize}%`}>
          {secondary}
        </Allotment.Pane>
      </Allotment>
    </div>
