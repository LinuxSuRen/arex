...
import { UpdateUserSettingsDocument } from '../../../helpers/backend/gen/graphql.ts';
...
const Settings = () => {
  const { properties, applySetting } = useSettings();
  const [cp] = useMutation(UpdateUserSettingsDocument);

  const { store, dispatch } = useContext(MainContext);
  const { EXTENSIONS_ENABLED, PROXY_ENABLED, PROXY_URL, LANGUAGE } =
    store.settings;
          ...
          >
            ...
            >
              ...
              <p>
                ...
                <span>{properties.THEME_COLOR}</span>
              </p>

              <CirclePicker
                width={'320px'}
                circleSize={20}
                color={properties.THEME_COLOR}
                colors={colors}
